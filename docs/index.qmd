...
def ensure(pip_name, import_name=None):
    """Instala pip_name si import_name (o pip_name) no está disponible."""
...
packages = [
    ...
    ("imbalanced-learn", "imblearn"),   # <- pip name correcto
    ("joblib", None),
    ("tabulate", None),
    ("openpyxl", None),

    # --- QR Code ---
    ("qrcode[pil]", "qrcode"),          # instala qrcode + pillow
    ("pillow", "PIL"),                  # asegura import PIL
]
...
def save_figure(name):
    """Save current figure to PNG with transparent background at 1200 DPI"""
    filename = f'figures/{name}.png'
    plt.savefig(filename, transparent=True, bbox_inches='tight', dpi=1200)

def save_table(df, name):
    """Save table to Excel"""
    global _table_counter
    _table_counter += 1
    filename = f'tables/{_table_counter:02d}_{name}.xlsx'
    if hasattr(df, 'to_excel'):
        df.to_excel(filename, index=True)
    elif hasattr(df, 'to_frame'):
        df.to_frame().to_excel(filename, index=True)
    else:
        try:
            df.data.to_excel(filename, index=True)
        except:
            pass
```
...
Unsupervised clustering of ICD‑10 codes yielded **12** clinically coherent comorbidity clusters with descriptive labels (e.g., *Post‑Procedural/Depression*, *Diabetes‑Hypertension/Anemia*, *Cardiac/Respiratory*, *Thrombosis/Effusion/Thyroid*, *Chemo‑Renal/Respiratory Failure*, *Malnutrition/Heart Failure*; see @tbl-clusters-description; @tbl-cluster-tables-12). The most prevalent clusters in the cohort were:
...
- **Thrombosis/Effusion/Thyroid** (**28.6%**, 10 codes) (see @tbl-clusters-description; @tbl-cluster-tables-12).

Cluster validity indices indicated overlapping profiles (Silhouette **0.068–0.094**, Davies–Bouldin **3.60–4.02**, Calinski–Harabasz ~**0.95–1.27**), consistent with multimorbidity in oncology populations (see @tbl-eval-metrics). Still, cluster size dispersion and dominant code frequencies supported interpretability at **k = 12** (see @tbl-comparison).

Although separation is modest (as expected in multimorbidity), the 12‑cluster solution provides clinically meaningful groupings that explain large portions of the comorbidity burden (see @fig-heatmaps-2).
...
plt.tight_layout()
save_figure('fig_temporal_trends')
plt.show()
plt.close()
```
...
plt.tight_layout()
save_figure('fig_comorbidities_trends')
plt.show()
plt.close()
```

### Evolution of service utilization and clinical severity

Hospital service mix shifted markedly toward higher‑acuity care (see @fig-services-severity). ICU assignments increased **~22×** (from **0.39%** in 2019 to **8.85%** in 2024), and ITU rose **~2.7×** (0.94%→2.51%), while “Other” services decreased (86.64%→77.69%). In parallel, the proportion of **High severity** admissions rose from **19.56%** to **26.00%** (+**6.45 pp**), **Low severity** declined (31.72%→23.05%, −**8.67 pp**), and **Medium** increased slightly (48.72%→50.94%, +**2.23 pp**) (see @fig-services-severity; @fig-temporal-trends).

Risk stratification mirrored these shifts: the share of patients in the **High mortality‑risk** stratum increased from **47.56%** to **57.14%** (+**9.59 pp**), with corresponding declines in **Medium** (−**6.87 pp**) and **Low** (−**2.71 pp**) risk (see @fig-temporal-trends).
...
Several clusters increased substantially from **2019→2024** (percentage‑point, pp, change among admissions; see @fig-comorbidities-trends):
...
Other clusters rose more modestly (**Neuropathy/GI Sensory** +8.48 pp; **Malnutrition/Heart Failure** +8.61 pp; **Liver‑Pelvic/Ascites** +3.35 pp; **Sepsis/Hemorrhagic/Anxiety** +3.29 pp; **Chemo‑Renal/Respiratory Failure** +2.15 pp). The multimorbidity landscape is intensifying—particularly cardio‑metabolic, thrombo‑embolic, and post‑procedural sequelae—aligning with the rising severity mix (see @fig-comorbidities-trends; @fig-services-severity).

Year‑specific **relative risks (RR)** of in‑hospital death showed strong heterogeneity across clusters, with consistent high‑risk signal in a few domains (see @fig-relative-risk-mortality):
...
## Predictive modelling: baselines, imbalance strategies, and discrimination

The dataset was partitioned into training and validation sets with preserved outcome prevalence (see @tbl-prepare-data and @tbl-train-test-split): **Training:** 7,870 episodes (deceased **n=320**, **4.07%**). **Validation:** 3,374 episodes (deceased **n=137**, **4.06%**). The overall class imbalance was approximately **1:23** (deceased : not‑deceased), as also reflected in the cohort summaries (see @tbl-general-summary).

We evaluated logistic regression and random forests under multiple sampling strategies (see @tbl-baseline-models; @tbl-balancing-techniques; @fig-metrics-comparison; @fig-roc-curves; @fig-eval-plots; @fig-detail-configs):
...
- **Imbalance‑aware approaches:**  
 ...
 Among tested options, **RF with ADASYN/SMOTE+Tomek** maximized **F1**; **RF with ROS/RUS** maintained strong **AUC** while achieving clinically meaningful recall (see @tbl-best-model-analysis; @fig-metrics-comparison; @fig-roc-curves; @fig-eval-plots).
...
### Operating points, threshold analysis, and error structure
Threshold sweeps (0.10–0.85) quantified the precision–recall trade‑off for the selected model (see @fig-threshold-analysis):
...
- Caseload increased and LOS decreased modestly while crude mortality remained ~**4%**, despite more **ICU/High‑severity** admissions (see @fig-temporal-trends; @fig-services-severity).
- Comorbidity burden intensified over time—especially **cardio‑metabolic**, **post‑procedural**, and **thrombo‑embolic** clusters (see @fig-comorbidities-trends).
- **Chemo‑Renal/Respiratory Failure**, **Thrombosis/Effusion/Thyroid**, and **Sepsis/Hemorrhagic** clusters carried the **highest mortality RR** (peaks **~6–16×**), despite not being the most prevalent (see @fig-relative-risk-mortality).
- Under extreme imbalance, **AUC alone is misleading**; **RF with ADASYN/SMOTE+Tomek** maximized **F1 ~0.27** at threshold 0.50, whereas **RF with ROS/RUS** preserved strong **AUC (≈0.86)** with higher recall options (see @tbl-best-model-analysis; @fig-metrics-comparison; @fig-roc-curves).
- Optimal **operating thresholds** depend on downstream action capacity; **0.25–0.30** is suitable for broad screening (high recall), whereas **0.70–0.75** suits targeted, resource‑intensive interventions (see @fig-threshold-analysis; @fig-confusion-matrix; @fig-key-findings).

# GitHub

```{python}
#| label: qr-code-transparent
#| fig-cap: "QR code for GitHub repository"
